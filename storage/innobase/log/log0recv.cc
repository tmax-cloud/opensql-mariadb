...
@param[in,out]	p	iterator which points to the records to apply to the certain
page
@return whether the page creation successfully */
static buf_block_t* recv_recovery_create_page_low(recv_sys_t::map::iterator& p)
{
  mtr_t mtr;
...
do_read:
    ...
    recs.state = recv_sys_t::recs_t::RECV_NOT_PROCESSED;
    return nullptr;
  }
  ...
  if ((log_sys.log.format & ~log_t::FORMAT_ENCRYPTED) != log_t::FORMAT_10_4 &&
...
buf_block_t* recv_recovery_create_page_low(const page_id_t page_id)
{
  ...
  if (p != recv_sys.pages.end()
      && p->second.state == recv_sys_t::recs_t::RECV_WILL_NOT_READ)
    block = recv_recovery_create_page_low(p);
  mutex_exit(&recv_sys.mutex);
  return block;
}
...
void recv_apply_hashed_log_recs(bool last_batch)
{
	...
	for (recv_sys_t::map::iterator p = recv_sys.pages.begin();
		...
		case recv_sys_t::recs_t::RECV_WILL_NOT_READ:
			if (!recv_recovery_create_page_low(p))
				goto apply;
		}
	...
	while (!recv_sys.pages.empty() || buf_get_n_pending_read_ios()) {
