                                       ...
                                       const row_import &cfg)
{
  ...
  const char *data_dir_path=
      DICT_TF_HAS_DATA_DIR(table->flags) ? table->data_dir_path : nullptr;

  char *filepath= fil_make_filepath(
      data_dir_path, {table->name.m_name, strlen(table->name.m_name)}, IBD,
  ...
  if (dberr_t err= os_file_read_no_error_handling(IORequestReadPartial, file,
                                                  first_page.get(), 0,
  ...
  const auto physical_size= zip_size ? zip_size : srv_page_size;
  ...
  if (table->supports_instant())
  {
    ...
    while (btr_page_get_level(page.get()) != 0)
    {
      ...
      if (dberr_t err= os_file_read_no_error_handling(
              IORequestReadPartial, file, page.get(),
    ...
    if (info_bits & REC_INFO_DELETED_FLAG)
    {
      ...
      std::unique_ptr<byte[], decltype(&aligned_free)> second_page(
          static_cast<byte *>(
              aligned_malloc(physical_size, UNIV_PAGE_SIZE_MAX)),
          &aligned_free);

      if (dberr_t err= os_file_read_no_error_handling(
              IORequestReadPartial, file, second_page.get(),
